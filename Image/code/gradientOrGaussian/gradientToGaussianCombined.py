...
def blur_and_highlight_edges(image_path, output_path):
    ...
    scharr_image = apply_scharr_filter(image)

    # Apply the Laplacian filter
    laplacian_image = apply_laplacian_filter(image)

    # Combine both filters
    combined_gradient = cv2.addWeighted(scharr_image, 0.5, laplacian_image, 0.5, 0)

    # Apply Gaussian blur to the combined gradient image
    blurred_image = cv2.GaussianBlur(combined_gradient, (51, 51), 0)
...
if __name__ == "__main__":
    ...
    output_directory = (
        r"C:\Users\Anuj Bohra\Desktop\IIT_Patna\anujwarp\combinedFilterImages"
    )
