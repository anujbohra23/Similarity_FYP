...
import json
import gradio as gr
from insanely_fast_whisper import WhisperModel  # Assuming this is the import path

# Load the large model
model = WhisperModel("large")  # Change "large" to the desired model size if applicable
os.makedirs("transcriptions", exist_ok=True)

print(WhisperModel.__version__)  # Print version for debugging


def transcribe(audio):
    # Load and preprocess the audio
    audio = model.load_audio(audio, sr=16000)  # Ensure audio is loaded at 16kHz
    ...
    options = model.DecodingOptions(fp16=False)
...
# Gradio application
gr.Interface(
    title="Insanely Fast Whisper Gradio App",
    fn=transcribe,
